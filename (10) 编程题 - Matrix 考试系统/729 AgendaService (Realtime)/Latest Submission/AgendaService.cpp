...
bool AgendaService::deleteUser(const std::string &userName, const std::string &password)
                     ...
                     const std::vector<std::string> &participator)
  {
    ...
    Date beg(startDate) , end(endDate) ;
    if(!Date::isValid(beg) || !Date::isValid(end))
    {
      return false ;
    }
    if( beg >= end )
    ...
    if(m_storage -> queryMeeting([&](const Meeting & a)
    {
      return a.getTitle() == title ;
    }).size() > 0 )  return false ;
    set<string> tmp ;
    if(m_storage->queryUser([&](const User &a)
    {
    ...
    if(tmp.size()!=participator.size())
    {
      return false;
    }
    Meeting new_meet(userName,participator,startDate,endDate,title) ;
    tmp.insert(userName) ;
    for(auto item : tmp )
    {
      auto res = m_storage -> queryMeeting([=] (const Meeting &m)
      {
        if(m.getSponsor() != item && !m.isParticipator(item))
        {
          return false ;
        }
        if(m.getStartDate()>=end || m.getEndDate()<=beg)
        {
          return false ;
        }
        else
    ...
    m_storage -> createMeeting(new_meet) ;
    return true ;
  }
                                  ...
                                  const std::string &endDate) const
  {
    ...
    rightmeeting= m_storage -> queryMeeting([&] (const Meeting & a) -> bool 
      {
          if(a.getSponsor() == userName || a.isParticipator(userName))
          {
            if((a.getStartDate() >= sTime && a.getEndDate() <= eTime ) ||
               (a.getStartDate() <= sTime && a.getEndDate() >= sTime ) ||
               (a.getStartDate() <= eTime && a.getEndDate() >= eTime ) ||
               (a.getStartDate() <= sTime && a.getEndDate() >= eTime ) )
            {
              return true ;
            }
            else
