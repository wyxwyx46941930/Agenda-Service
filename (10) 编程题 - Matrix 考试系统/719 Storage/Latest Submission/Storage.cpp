...
using namespace std ;
  ...
  */
  bool Storage::writeToFile(void)
  {
        ...
        ofstream out1 ;
        out.open("user.txt") ;
        out1.open("meeting.txt") ;
       // out1.open(Path::meetingPath) ;
        ...
        for( auto it = m_meetingList.begin() ; it != m_meetingList.end() ; it ++ )
        {
          if( it != it2 )
          {
            ...
            out1 << "\"" << "," ;
            out1 << "\"" << Date::dateToString( it -> getStartDate()) << "\"" << "," ;
            out1 << "\"" << Date::dateToString( it -> getEndDate()) << "\"" << "," ;
            out1 << "\"" << it -> getTitle() << "\"" ;
            out1 << endl ;
          }
          else
